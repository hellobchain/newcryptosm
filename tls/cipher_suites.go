...
const (
	...
	suiteECDHE = 1 << iota
	// suiteSM2E indicates that the cipher suite involves elliptic curve
	// Diffie-Hellman. This means that it should only be selected when the
	// client indicates that it supports SM2 with a curve and point format
	// that we're happy with.
	suiteSM2E
	...
	// RSA based.
	suiteECDSA
	// suiteSM2 indicates that the cipher suite involves an SM2
	// signature and therefore may only be selected when the server's
	// certificate is SM2.
	suiteSM2
...
var mapFlags = map[int]string{
	suiteECDHE:      "suiteECDHE",
	suiteSM2E:       "suiteSM2E",
	suiteECDSA:      "suiteECDSA",
	suiteSM2:        "suiteSM2",
	suiteTLS12:      "suiteTLS12",
	suiteSHA384:     "suiteSHA384",
	suiteDefaultOff: "suiteDefaultOff",
}
...
func (c *cipherSuite) String() string {
	var flagsStr string
	var flags = []int{suiteECDHE, suiteSM2E, suiteECDSA, suiteSM2, suiteTLS12, suiteSHA384, suiteDefaultOff}
	for _, flag := range flags {
		if c.flags&flag != 0 {
			flagsStr = fmt.Sprintf("%s|%v", flagsStr, mapFlags[flag])
		}
	}
	return fmt.Sprintf("id:%v\nkeyLen:%v\nmacLen:%v\nivLen:%v\nflags:%v\n",
		mapId[c.id], c.keyLen, c.macLen, c.ivLen, flagsStr)
}
...
var cipherSuites = []*cipherSuite{
	...
	{TLS_ECDHE_SM2_WITH_SM4_SM3, 16, 20, 16, ecdheSM2KA, suiteECDHE | suiteSM2 | suiteTLS12, cipherSM4, macSM3, nil},
	{TLS_SM2_WITH_SM4_SM3, 16, 20, 16, sm2KA, suiteSM2E | suiteSM2 | suiteTLS12, cipherSM4, macSM3, nil},
	{TLS_SM4_GCM_SM3, 16, 0, 4, sm2KA, suiteSM2E | suiteSM2 | suiteTLS12, nil, nil, aeadSM4GCM},
	{TLS_SM4_CCM_SM3, 16, 0, 4, sm2KA, suiteSM2E | suiteSM2 | suiteTLS12, nil, nil, aeadSM4GCM},
...
// macSM3 returns a SM3 based MAC. These are only supported in TLS 1.2
// so the given version is ignored.
...
const (
	...
	TLS_ECDHE_SM2_WITH_SM4_SM3              uint16 = 0xe031 // wsw add
	TLS_SM2_WITH_SM4_SM3                    uint16 = 0xe033 // wsw add
	TLS_SM4_GCM_SM3                         uint16 = 0x00c6 // wsw add
	TLS_SM4_CCM_SM3                         uint16 = 0x00c7 // wsw add
...
var mapId = map[uint16]string{
	...
	TLS_ECDHE_SM2_WITH_SM4_SM3:              "TLS_ECDHE_SM2_WITH_SM4_SM3", // wsw add
	TLS_SM2_WITH_SM4_SM3:                    "TLS_SM2_WITH_SM4_SM3",       // wsw add
	TLS_SM4_GCM_SM3:                         "TLS_SM4_GCM_SM3",            // wsw add
	TLS_SM4_CCM_SM3:                         "TLS_SM4_CCM_SM3",            // wsw add
	// TLS_FALLBACK_SCSV isn't a standard cipher suite but an indicator
	// that the client is doing version fallback. See
	// https://tools.ietf.org/html/rfc7507.
