...
func (c *Conn) clientHandshake() error {
	...
	// client hello (random cipher suites supported proto version)
	hello := &clientHelloMsg{
...
NextCipherSuite:
	...
	if hello.vers >= VersionTLS12 {
		hello.signatureAndHashes = supportedSignatureAlgorithms // sm3
	}
	...
	// send server client hello
	if _, err := c.writeRecord(recordTypeHandshake, hello.marshal()); err != nil {
		return err
	}

	// read server hello
	msg, err := c.readHandshake() // serverHello
	if err != nil {
		return err
	}
...
func (hs *clientHandshakeState) doFullHandshake() error {
	...
	if c.handshakes == 0 {
		...
		certs := make([]*x5092.Certificate, len(certMsg.certificates))
		for i, asn1Data := range certMsg.certificates {
			cert, err := x5092.ParseCertificate(asn1Data)
			if err != nil {
		...
		if !c.config.InsecureSkipVerify {
			opts := x5092.VerifyOptions{
				Roots:         c.config.RootCAs,
				CurrentTime:   c.config.time(),
				DNSName:       c.config.ServerName,
				Intermediates: x5092.NewCertPool(),
			}
	...
	if chainToSend != nil && len(chainToSend.Certificate) > 0 {
		...
		case *ecdsa.PublicKey:
			...
			if ecdsa.IsSM2(pub.Curve.Params()) {
...
var tls11SignatureSchemes = []SignatureScheme{SM2WithP256AndSM3}
...
const (
	...
	// tls11SignatureSchemesNumRSA is the number of trailing elements of
	// tls11SignatureSchemes that use SM2.
	tls11SignatureSchemesNumSM2 = 5
)
...
func (hs *clientHandshakeState) getCertificate(certReq *certificateRequestMsg) (*Certificate, error) {
	...
	if c.config.GetClientCertificate != nil {
		...
		if !certReq.hasSignatureAndHash {
			...
			if !sm2Avail { // wsw add
				signatureSchemes = signatureSchemes[:len(signatureSchemes)-tls11SignatureSchemesNumSM2]
			}
		} else {
...
findCert:
	for i, chain := range c.config.Certificates {
		if !rsaAvail && !ecdsaAvail && !sm2Avail { // wsw add
			continue
		}
		...
		for j, cert := range chain.Certificate {
			...
			if j != 0 || x509Cert == nil {
				var err error
				if x509Cert, err = x5092.ParseCertificate(cert); err != nil {
					c.sendAlert(alertInternalError)
			...
			switch {
			case rsaAvail && x509Cert.PublicKeyAlgorithm == x5092.RSA:
			case ecdsaAvail && x509Cert.PublicKeyAlgorithm == x5092.ECDSA:
			case sm2Avail && x509Cert.PublicKeyAlgorithm == x5092.SM2:
			default:
				continue findCert
			}
