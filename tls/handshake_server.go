...
func (hs *serverHandshakeState) readClientHello() (isResume bool, err error) {
	c := hs.c
	// read client hello from client
	msg, err := c.readHandshake()
	if err != nil {
		return false, err
	}
	// deal with client hello
	var ok bool
...
func (hs *serverHandshakeState) doFullHandshake() error {
	...
	if len(c.peerCertificates) > 0 {
		...
		if certVerify.hasSignatureAndHash {
			...
			switch pub.(type) {
			case *ecdsa.PublicKey:
				pb := pub.(*ecdsa.PublicKey)
				if ecdsa.IsSM2(pb.Params()) {
		...
		switch key := pub.(type) {
		case *ecdsa.PublicKey:
			if signatureAndHash.signature != signatureSM2 && signatureAndHash.signature != signatureECDSA {
		...
		case *rsa.PublicKey:
			...
			err = rsa.VerifyPKCS1v15(key, crypto.Hash(hashFunc), digest, certVerify.signature)
		}
		if err != nil {
			c.sendAlert(alertBadCertificate)
...
func (hs *serverHandshakeState) processCertsFromClient(certificates [][]byte) (crypto.PublicKey, error) {
	...
	certs := make([]*x5092.Certificate, len(certificates))
	var err error
	for i, asn1Data := range certificates {
		if certs[i], err = x5092.ParseCertificate(asn1Data); err != nil {
			c.sendAlert(alertBadCertificate)
	...
	if c.config.ClientAuth >= VerifyClientCertIfGiven && len(certs) > 0 {
		opts := x5092.VerifyOptions{
			...
			KeyUsages:     []x5092.ExtKeyUsage{x5092.ExtKeyUsageClientAuth},
		}
...
func (hs *serverHandshakeState) setCipherSuite(id uint16, supportedCipherSuites []uint16, version uint16) bool {
	for _, supported := range supportedCipherSuites {
		if id == supported {
			...
			if candidate.flags&suiteECDHE != 0 { // 都是ecdhe密钥交换
				if !hs.ellipticOk { // 是不是椭圆曲线 若是
					continue
				}
				if candidate.flags&suiteECDSA != 0 { // 再判断是不是 签名算法是ecdsa
					if !hs.ecdsaOk {
						continue
					}
			...
			} else if candidate.flags&suiteSM2E != 0 { // sm2 ex
				if !hs.ellipticOk { // 是不是椭圆曲线 若是
					continue
				}
				if candidate.flags&suiteSM2 != 0 { // 再判断是不是 签名算法是SM2
					if !hs.sm2Ok {
						continue
					}
				}
			} else if !hs.rsaDecryptOk { // rsa ex
				continue
			}
