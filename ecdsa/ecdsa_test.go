...
func TestKeyGeneration(t *testing.T) {
	...
	testKeyGeneration(t, sm2.SM2(), "SM2")
}
...
func BenchmarkSignSM2(b *testing.B) {
	...
	sm2 := sm2.SM2()
	hashed := []byte("testing")
	priv, _ := GenerateKey(sm2, rand.Reader)
...
func TestSignAndVerify(t *testing.T) {
	...
	testSignAndVerify(t, sm2.SM2(), "SM2")
}
...
func TestNegativeInputs(t *testing.T) {
	...
	testNegativeInputs(t, sm2.SM2(), "SM2")
}

func testBatchSignAndVerify(t *testing.T, c elliptic.Curve, tag string) {
	for i := 0; i < 1000000; i++ {
		priv, _ := GenerateKey(c, rand.Reader)

		hashed := make([]byte, 16)
		io.ReadFull(rand.Reader, hashed)
		r, s, err := Sign(rand.Reader, priv, hashed)
		if err != nil {
			t.Errorf("%s: error signing: %s", tag, err)
			return
...
func TestBatchSignAndVerify(t *testing.T) {
	testSignAndVerify(t, elliptic.P224(), "p224")
	if testing.Short() {
		return
	}
