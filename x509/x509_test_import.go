...
func main() {
	...
	rsaPriv, err := x5092.ParsePKCS1PrivateKey(block.Bytes)
	if err != nil {
		panic("Failed to parse private key: " + err.Error())
	}

	template := x5092.Certificate{
		...
		KeyUsage:  x5092.KeyUsageCertSign,
	}

	if _, err = x5092.CreateCertificate(rand.Reader, &template, &template, &rsaPriv.PublicKey, rsaPriv); err != nil {
